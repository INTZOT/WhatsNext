# Changelog

## [Unreleased]

### 1.2.0 — 登录保持 · 成员可见性 · 部署修复

**新功能**
- 登录保持 14 天：会话有效期显式设为 14 天，访问期间自动续签（滑动窗口，14 天不活动才过期）
- 非管理员成员可查看清单成员列表（只读），添加/移除成员等管理操作仅管理员可见

**修复**
- 更换占位符 `AUTH_SECRET` 为随机密钥，解决"每次打开网页都要重新登录"问题
- 迁移文件正式纳入版本控制（此前被 `.gitignore` 忽略），修复全新部署时 `prisma migrate deploy` 报 drift 要求 reset 的问题
- README 重写：Docker 部署 + 本地部署完整说明

**升级注意**
- 本次更换了 `AUTH_SECRET`，升级后所有用户需要重新登录一次

### 1.1.3 — 加入清单 · 退出清单 · 部署优化

**新功能**
- 仪表盘「加入清单」按钮：输入清单 ID 直接加入为参与者
- 清单详情页「退出清单」按钮（管理者和参与者可退出）
- 清单 ID 显示与一键复制
- 新建任务支持描述字段

**修复**
- `dueDate` 和 `notes` 空值验证修复（Zod `.nullable().optional()`）
- 登录页 `useSearchParams` Suspense 包裹
- NextAuth `next-auth/jwt` 模块声明兼容性修复
- ESLint `no-explicit-any` 降级为 warning
- 复制按钮 HTTP 兼容（`document.execCommand` 回退）

### 1.1.2 — 安全加固

**漏洞修复**
- 成员邮箱仅管理员可见（GET members / GET list 端点）
- 用户搜索端点限制为清单管理员，需传 `listId` 参数
- 创建任务时验证 `assigneeId` 为清单成员
- 创建任务时验证 `parentTaskId` 属于同一清单
- PATCH 任务时验证新 `assigneeId` 为清单成员
- 注册错误信息泛化，消除用户枚举漏洞

**安全增强**
- 密码复杂度要求：至少一个大写字母 + 一个数字
- `taskQuerySchema` 改用 `.safeParse()` 防止 500 崩溃
- 标签名统一 `.trim().filter(Boolean)` 处理
- NextAuth 配置 `trustHost: true`（反向代理支持）
- 添加 MIT 开源许可证

### 1.1.1 — Logo · 品牌

**品牌**
- 新增导航栏 Logo（`public/logo.png`），支持暗色模式自动反色（`dark:invert`）
- 项目正式更名为 **WhatsNext**
- 浏览器 favicon 配置（`src/app/icon.png`）
- Open Graph 社交分享图片配置

### 1.1.0 — 看板 · 子任务 · 标签 · 任务详情

**看板视图**
- 三列看板（待办 / 进行中 / 已完成）
- 列表 / 看板一键切换
- 看板卡片点击进入任务详情
- 下拉菜单快速移动任务状态

**子任务**
- 一层嵌套子任务，父任务下折叠展示
- 父任务标记完成 → 级联完成所有子任务
- 子任务独立支持状态切换、标签、负责人
- 任务详情页管理子任务（添加 / 删除 / 状态切换）

**标签系统**
- 清单级别标签（Tag + TaskTag 多对多）
- 创建 / 编辑任务时内联添加标签（回车或按钮）
- 任务卡片和详情页展示标签
- 工具栏标签下拉筛选

**任务详情页**
- 点击任务卡片进入独立详情页
- 编辑模式：标题、状态、优先级、截止日期、负责人、描述、标签
- 子任务列表管理
- 删除任务（确认弹窗）
- 截止日期为空显示「无」

**权限优化**
- 前端 UI 门控：编辑/删除按钮仅创建者和管理者可见
- 任务详情页根据用户角色展示对应操作
- 子任务删除仅管理员可操作
- 参与者可切换自己任务状态、添加子任务

**UI 修复**
- 文本长度限制：标题 60 / 标签 30 / 清单名 50 字符
- 全量 `overflow-hidden` + `break-words` 防止文本溢出
- 标签筛选按钮长文本省略号截断
- 编辑对话框回车键仅添加标签，不触发表单提交
- 成员搜索排除已有成员
- 成员添加改为 userID 精确匹配
- 排序下拉暗色模式可见性修复
- 清单描述支持展开/收起
- 创建者可见删除清单按钮

### 1.0.0 — 团队待办清单 MVP

**基础设施**
- Next.js 15 App Router + React Server Components
- TypeScript 严格模式
- Tailwind CSS + shadcn/ui 组件库
- PostgreSQL + Prisma ORM 数据持久化
- NextAuth.js v5 邮箱密码认证
- Docker Compose 一键部署
- pnpm 包管理

**账号与认证**
- 邮箱 + 密码注册与登录
- NextAuth.js Session 管理
- CSRF 保护

**清单管理**
- 创建清单（成员自动成为创建者）
- 编辑清单名称与描述
- 删除清单（仅创建者）

**权限系统**
- 三级角色：创建者 / 管理者 / 参与者
- 应用层中间件校验（Next.js Middleware + API Route Guards）
- 创建者可转让创建者角色、修改成员角色
- 管理者可添加/移除成员、管理任务
- 参与者仅操作自己负责的任务
- 无权限操作统一返回 403

**任务管理**
- 任务 CRUD
- 固定三态：待办 → 进行中 → 已完成
- 优先级：高 / 中 / 低
- 截止日期
- 指派负责人
- 排序：优先级 / 截止日期 / 创建时间
- 筛选：按状态、按指派人、「只看我的」
- 标题关键词搜索

**成员管理**
- 组合搜索（用户名 / 显示名 / 邮箱）添加成员
- 实时成员列表

**UI / UX**
- 响应式布局（桌面 / 平板 / 手机）
- 仪表盘（所有参与清单概览）
- 清单详情页（任务列表 + 成员面板）
- 暗色模式
